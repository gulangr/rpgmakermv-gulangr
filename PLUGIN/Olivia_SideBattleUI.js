...
Window_Help.prototype.refreshDimmerBitmap = function() {
    // 检查是否存在淡化层精灵（_dimmerSprite），如果不存在则不执行后续操作(RYQ改)
    if (this._dimmerSprite) {
        ...
        // 计算第一部分矩形的宽度（窗口宽度的一半，向上取整）
        var w1 = Math.ceil(w / 2);
        // 计算第二部分矩形的宽度（第一部分宽度减去侧边UI状态区域的宽度）
        var w2 = w1 - Olivia.OctoBattle.SideBattleUI.StatusWidth;
        // 计算绘制区域的高度（窗口高度减去上下两边的内边距）
        var h1 = h - m * 2;
        // 获取第一种淡化颜色（用于纯色填充和渐变起始色）
        var c1 = 'rgba(0, 0, 0, 0.35)'; // 淡化颜色1
        // 获取第二种淡化颜色（用于渐变结束色）
        var c2 = 'rgba(0, 0, 0, 0)'; // 淡化颜色2
        // 调整位图的大小以匹配窗口尺寸
        bitmap.resize(w, h);
        // 在指定区域绘制纯色矩形（左半部分）
        bitmap.fillRect(0, m, w1, h1, c1);
        // 在指定区域绘制渐变色矩形（右半部分，从c1过渡到c2）
...
Window_ActorCommand.prototype.updatePosition = function() {
    if (this._actor) {
        ...
        //RYQ改 RYQ注释 这里更改actorcommand窗口相对于角色sprite的位置
        this.x = x + (this._positionXCorrection || 0) -74;
        this.y = y + (this._positionYCorrection || 0) -110;
        Window_PartyCommand.prototype.updateFadeIn.call(this);
        Window_PartyCommand.prototype.correctScreenPosition.call(this);
    }
};
...
Window_BattleSideStatus.prototype.initialize = function(index) {
    ...
    //RYQ改 默认是没有-50
    var x = Graphics.boxWidth - width -18;
...
// 定义Window_BattleSideStatus类的refresh方法，用于刷新窗口内容（RYQ改）
Window_BattleSideStatus.prototype.refresh = function() {
    // 清除窗口内容区域，准备重新绘制
    this.contents.clear();
    // 检查是否存在当前关联的角色（_actor），如果存在则执行以下绘制逻辑
    if (!!this._actor) {
        // 定义全透明颜色，rgba格式中最后一个参数0表示完全透明
        var transparent = 'rgba(128, 128, 128, 0.2)';
        var transparent1 = 'rgba(128, 128, 128, 0)';
        // 计算第一个区域的宽度，为窗口宽度的1/4并向上取整
        var w1 = Math.ceil(this.width / 4);
        // 计算第二个区域的宽度，为窗口总宽度减去第一个区域的宽度
        var w2 = this.width - w1;
        // 获取窗口的高度
        var h = this.height;
        // 在内容区域绘制一个渐变矩形（此处起止颜色相同，效果等同于纯色矩形）
        // 绘制范围：从(0,0)到(w1, h)，使用全透明颜色
        this.contents.gradientFillRect(0, 0, w1, h, transparent1, transparent);
        // 在内容区域绘制一个纯色矩形
        // 绘制范围：从(w1, 0)到(w2, h)，使用全透明颜色
        this.contents.fillRect(w1, 0, w2, h, transparent);
    }
};
