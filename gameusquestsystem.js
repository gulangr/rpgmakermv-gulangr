...
//---------------------------------------------------------------------------------------------
    ...
    Window_QuestInfo.prototype.createQuestBitmap = function() {
        ...
        // --- 开始修改：设置字体大小 ---
        var customFontSize = 20;  // RYQ改在这里修改成你想要的字体大小（默认通常是28）
        this.questBitmap.fontSize = customFontSize; // 设置显示文字的大小
        this.contents.fontSize = customFontSize;    // 重要：设置测量文字的大小，否则换行位置会错乱
        // --- 结束修改 ---
        if (this.quest > 0) {
    ...
    Window_QuestInfo.prototype.drawQuestSteps = function(q) {
        ...
        this.questBitmap.textColor = this.systemColor(); 
        var stepsWord = GameusScripts["Config"]["QuestSystem"]["Steps Word"] || "Steps";
        this.questBitmap.drawText(stepsWord, 0, this.lineY, this.contentsWidth(), this.lineHeight());
        this.write();
        this.questBitmap.textColor = this.normalColor();
    ...
    Window_QuestInfo.prototype.drawQuestRewards = function(q) {
        ...
        // Draw Rewards
        this.questBitmap.textColor = this.systemColor(); 
        var rewardsWord = GameusScripts["Config"]["QuestSystem"]["Rewards Word"] || "Rewards";
        this.questBitmap.drawText(rewardsWord, 0, this.lineY, this.contentsWidth(), this.lineHeight());
        this.write();
        this.questBitmap.textColor = this.normalColor();
